...
while True:
    ...
    if agent.spec_heat < abs(agent.delta_heat_signal):
        ...
        phi = random.choice([-pi/3, pi/3, 0],  weights=[2,2,80], k=1)
        dx = sin(phi)
        dz = dx
    ...
    if not (-side+w < agent.pos.x < side+R-d) or agent.pos.z in hexagon.pos.x:
        agent.p.x = -agent.p.x
        print("Crashed! on x")

    if not (-side+w < agent.pos.z < side+R-d) or agent.pos.z in hexagon.pos.z:
        agent.p.z = -agent.p.z
        print("Crashed! on z")
